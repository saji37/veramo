...
router.post( "/create-vc", async ( req: { body: { connectionid: string; issuerDid: string; name: string; dob: string; address: string; }; }, res: any ) => {
    const { connectionid, issuerDid, name, dob, address } = req.body;
    try {
      // const holderDid = await findHolder(parseInt(holderId));
      const vc = await createVc(issuerDid, name, dob, address);
